...
TEST_F(ArgumentsParserTest, invalid_flag) {
    char *args[] = {"--fie"};
    EXPECT_THROW(parse_arguments(1, args, parsed_config), std::invalid_argument);
}

TEST_F(ArgumentsParserTest, invalid_argument) {
    char *args[] = {"--mode", "qwe", "123"};
    EXPECT_THROW(parse_arguments(3, args, parsed_config), std::invalid_argument);
}
...
TEST_F(ArgumentsParserTest, invalid_number) {
    char *args[] = {"--mode", "max", "qwe"};
    EXPECT_THROW(parse_arguments(3, args, parsed_config), std::invalid_argument);
}
